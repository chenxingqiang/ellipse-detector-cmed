...
/* 椭圆检测参数设置(阀值-实际与理想值的差值)：我的限定条件123、直弧检测阀值、弧组合筛选阀值 */
bool myselect1=true;
bool myselect2 = false;
bool myselect3 = false;	// TODO：select的作用是开关吗，这里设为false是为了放松约束吗？
float tTCNl = 0.05f;//过滤太直的曲线
//float tCNC=0.3f;
float tCNC = 2.97f;	// 原:0.34f~0.47f; 
	// 改a:100.36f; // 改b:0.52f; // 改c1:20.26f~20.57f; // 改c2:2.97f;

// 存储选择组合前的合格的弧
Mat DPDN_temp_only_wan_arc;
...
void CNEllipseDetector::DetectEdges13(Mat1b& DP, VVP& points_1, VVP& points_3)
{
	...
	for (int i = 0; i < iContoursSize; ++i)
	{
		...
		// 如果能走到这里，说明这个弧是足够弯的弧，把这个弧纪录下来（13象限内的弧）
		/*for (int kl = 0; kl < edgeSegment.size(); kl++) {	// 对于这条弧的每一个点
			DPDN_temp_only_wan_arc.at<uchar>(edgeSegment[kl].y, edgeSegment[kl].x) = 255;
		}*/
...
void CNEllipseDetector::DetectEdges24(Mat1b& DN, VVP& points_2, VVP& points_4 )
{
	...
	for (int i = 0; i < iContoursSize; ++i)
	{
		...
		// 如果能走到这里，说明这个弧是足够弯的弧，把这个弧纪录下来（24象限内的弧）
		/*for (int kl = 0; kl < edgeSegment.size(); kl++) {	// 对于这条弧的每一个点
			DPDN_temp_only_wan_arc.at<uchar>(edgeSegment[kl].y, edgeSegment[kl].x) = 255;
		}*/
...
//#define T124 pil,pim,pif,pjl,pjm,pjf // 下一行的定义是原来的《不推荐使用宏定义》
...
/* TODO：为什么六点需要有顺序(理论上求映射点判共线不需要顺序)？为什么V2SP的排列为(p3, p2, p1)-(p4, p5, p6)？ */

// VP为点集合，即代表一个弧，弧上点的排列按<左→右的方向>排列；VVP为点集合的集合，即代表一个弧的集合。
// unordered_map类似HashMap，以hash值判断元素是否相等，内部元素无序，相较于HashMap前者支持string或复杂对象做key元素。

/* 分别在124象限中的弧集合(pi, pj, pk)中各取一个，组合后进行几何约束和同圆判断，满足两筛选条件的加入候选组合的集合中。 */
									   ...
									   )
{
	...
	for (ushort i = 0; i < sz_i; ++i)
	{
		...
		for (ushort j = 0; j < sz_j; ++j)
		{
			...
			/* 弧arc_j上取三点：左端点+弧中点+右端点(左x最小 < 右x最大) */
			Point& pjf = edge_j[0];
			Point& pjm = edge_j[sz_ej/2];
			Point& pjl = edge_j[sz_ej - 1];

#ifndef DISCARD_CONSTRAINT_POSITION
			/* 几何约束验证-1：2象限中弧上点的x坐标 < 1象限中弧上点的x坐标； */
			/* 若2.x > 1.x，即不满足约束条件则跳过，进入下一条弧的判断。(1.x+1是误差范围) */
...
#endif
/* 这里是我需要改动的value4SixPoints：更换两弧上六点判断同圆的方法，看效率是否有提升！ */
#ifndef DISCARD_CONSTRAINT_CNC
			/* 同圆判断-0：弧arci-1象限和弧arcj-2象限上的分别三点(共六点)，计算特征值CNC，满足阀值范围内则两弧同圆； */
			/* 若 |T124=12的特征值 - 1| > thCNC(=0.3)，即不满足阀值范围，则两弧不同圆，跳过进入下一条弧的判断。(0.3是误差范围，严格说T124的CNC值应该为1) */
			/* 三个myselect条件：应该是控制'此次运行时某个两邻域弧判断同圆操作是否生效'的开关 */
			// 添加限定条件  2se se1//pil,pim,pif,pjf,pjm,pjl或pjf,pjm,pjl,pif,pim,pil
			if(myselect1&&fabs(value4SixPoints(T124))>tCNC)
			{
				continue;
			}
#endif
			/* 若满足上述两个约束条件，则符合这两弧候选情况，根据ij索引值和12象限key计算其HasnTable的存储key，以便后续插入候选组合集合中。 */
			uint key_ij = GenerateKey(PAIR_12, i, j);

			/* 3-最后遍历另一邻象限的弧集合(4象限-pk) */
			//for each edge k
			for (ushort k = 0; k < sz_k; ++k)
			{
				...
				/* 弧arc_k上取三点：左端点+弧中点+右端点(左x最小 < 右x最大) */
				Point& pkf = edge_k[0];
				Point& pkm = edge_k[sz_ek/2];
				Point& pkl = edge_k[sz_ek - 1];

#ifndef DISCARD_CONSTRAINT_POSITION
				/* 几何约束验证-2：4象限中弧上点的y坐标 > 1象限中弧上点的y坐标； */
				/* 若4.y < 1.y，即不满足约束条件则跳过，进入下一条弧的判断。(1.y-1是误差范围) */
				//CONSTRAINTS on position
...
#ifndef DISCARD_CONSTRAINT_CNC
				/* 同圆判断-0：弧arci-1象限和弧arck-4象限上的分别三点(共六点)，计算特征值CNC，满足阀值范围内则两弧同圆； */
				/* 若 |T124=14的特征值 - 1| > thCNC(=0.3)，即不满足阀值范围，则两弧不同圆，跳过进入下一条弧的判断。(0.3是误差范围，严格说T124的CNC值应该为1) */
				// 添加限定条件2		pif,pim,pil,pkf,pkm,pkl
				if(myselect2&&fabs(value4SixPoints(pif, pim, pil, pkf, pkm, pkl))>tCNC)
				{
					continue;
				}
				/* 同圆判断-0：弧arcj-2象限和弧arck-4象限上的分别三点(共六点)，计算特征值CNC，满足阀值范围内则两弧同圆； */
				/* 若 |T124=24的特征值 - 1| > thCNC(=0.3)，即不满足阀值范围，则两弧不同圆，跳过进入下一条弧的判断。(0.3是误差范围，严格说T124的CNC值应该为1) */
				// 添加限定条件3		pjf,pjm,pjl,pkf,pkm,pkl
				if(myselect3&&fabs(value4SixPoints(pjf, pjm, pjl, pkf, pkm, pkl))>tCNC)
				{
					continue;
				}
#endif
				...
				/* data是无序Map集合(筛选后的候选弧组合)，其中key值由GenerateKey()函数生成，value为EllipseData类型的对象。 */
				/* 根据弧arci-1和弧arcj-2找到圆心O12-data_ij，以及根据弧arci-1和弧arck-4找到圆心O14-data_ik */
				// Find centers
				EllipseData data_ij, data_ik;

				/* 判断key_ij对应的EllipseData元素data_ij是否已在集合中； */
				// If the data for the pair i-j have not been computed yet
				if (data.count(key_ij) == 0)
				{
					/* 若不在集合中，则计算出data_ij的相关内容(圆心O12等)，并插入集合中 */
				...
				{
					/* 若已在集合中，则根据key值取出对应的data_ik元素(圆心O14等相关内容在插入集合之前已经计算好了) */
					
					// Otherwise, just lookup the data in the hash table
					data_ik = data.at(key_ik);
				}
				
				/* 需保证data_ij和data_ik皆为有效内容 */
				// INVALID CENTERS
				...
				/* 需满足圆心位置约束：O12和O14距离相近；严格说因为arci和arcj同圆且arci和arck同圆，则O12和O14应该重合，目前只需二者在误差范围内即可。 */
...
#endif
				/* 若上述所有约束：每两弧的几何约束+同圆判断，两圆心O12和O14的位置约束；均已满足，则可根据上述结果估计圆心坐标和长短轴等参数。 */
									   ...
									   )
{
	...
	for (ushort i = 0; i < sz_i; ++i)
	{
		...
		for (ushort j = 0; j < sz_j; ++j)
		{
			...
			if(myselect1&&fabs(value4SixPoints(T231))>tCNC)
			{
				continue;
			}
#endif
									   ...
									   )
{
	...
	for (ushort i = 0; i < sz_i; ++i)
	{
		...
		for (ushort j = 0; j < sz_j; ++j)
		{
			...
			if(myselect1&&fabs(value4SixPoints(T342))>tCNC)
			{
				continue;
			}
#endif
									   ...
									   )
{
	...
	for (ushort i = 0; i < sz_i; ++i)
	{
		...
		for (ushort j = 0; j < sz_j; ++j)
		{
			...
			if(myselect1&&fabs(value4SixPoints(T413))>tCNC)
			{
				continue;
			}
#endif
...
void CNEllipseDetector::Detect(Mat1b& I, vector<Ellipse>& ellipses)
{
	...
	/*使用封装的函数Tic()首-Toc()尾记录每一步的运行时间(ms)到时间数组_times中：
	    [边缘检测, 预处理, 分组, 预估, 校验, 聚类]。*/
	...
	/* 保存此图片13+24象限内合格的弧：初始化，纪录13象限+纪录24象限，保存写入图片 */
	/*DPDN_temp_only_wan_arc = DP.clone();
	int rows = DP.rows;  //图片的宽
	int cols = DP.cols;  //图片的高
	for (int i = 0; i < rows; i++)
		for (int j = 0; j < cols; j++)
		{
			DPDN_temp_only_wan_arc.at<uchar>(i, j) = 0;
		}*/

	DetectEdges13(DP, points_1, points_3);
	DetectEdges24(DN, points_2, points_4);

	/*imwrite("./result/合格的弧.jpg", DPDN_temp_only_wan_arc);*/
	/* 保存此图片13+24象限内合格的弧：结束*/
...
//	IplImage ttt = (IplImage)tmp2;
//	IplImage *tt2 = &ttt;
//	cvShowImage("DNP", tt2);
	IplImage tmp3 = (IplImage)tmp2;
	IplImage tmp4 = IplImage(tmp2);
	cvShowImage("DNP", &tmp3);
//	CvArr tmp4 = IplImage(tmp3);
//	cvSaveImage("DPN.jpg",&IplImage(tmp2));
 	cvSaveImage("DPN.jpg",&tmp4);
//  
	//end show DpDn
	...
	imgs[2] = picture3;
	imgs[3] = picture4; 
	IplImage picture5_1 = (IplImage)picture5;
	IplImage picture5_2 = IplImage(picture5);
	cvNamedWindow("all arcs", CV_WINDOW_NORMAL);  
//	cvShowImage("all arcs",&(IplImage)picture5 );
	cvShowImage("all arcs", &picture5_1);
	cvSaveImage("arcs.jpg", &picture5_2);
...
void CNEllipseDetector:: showAllEdgeInPic(Mat1b& I){
	...
	IplImage DP_1 = (IplImage)DP;
	cvShowImage("aaas", &DP_1);
...
/* 代码改动：
1、line-27、72：最优化参数，tCNC = 0.47f；score = 0.61是_fMinScore = 0.61吗？
2、line-1147：124+231+342+413方法中的阀值判断时的'-1'操作换到value4SixPoints方法返回result值时进行；
3、把DISCARD_TCN和DISCARD_TCN1的宏定义去掉，改为DISCARD_TCN2宏定义。
*/