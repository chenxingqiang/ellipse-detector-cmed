...
%     dataset = 'Dataset#3';  % Dataset#2、Dataset#1、Calibration
%     dataset = 'Prasad_plus';%Prasad Dataset，Prasad_plus
%     dataset = 'Industrial PCB Image Dataset';
     dataset = 'Tableware';%Tableware,Traffic Sign Dataset
    files=dir(['./EllipseDataset_Lu/', dataset, '/gt0/*.txt']); %需改
    path_GT=(['./EllipseDataset_Lu/', dataset, '/gt0/']); %需改
%     % 从*.mat文件中加载检测椭圆集数据
...
%     algoParamTmp = Ellipses_CNC;
    % 从det_*.txt文件中读取检测椭圆集数据
    filesDet=dir(['./EllipseDataset_Lu/', dataset, '/resultDet/*.txt']);
    path_Det=(['./EllipseDataset_Lu/', dataset, '/resultDet/']);
